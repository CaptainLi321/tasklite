...
## 上下文
下游业务项目（如 `platform_agrregate`、`douyin_tp`、`aryion_tp`、`bangumi_id`）普遍存在 HTTP 网络请求、429 限流退避、Netscape Cookies 解析与原始响应快照缓存需求。由于不同平台对于请求库（`urllib`、`requests`、`httpx`、自定义签名 SDK）偏好各异，若由 TaskLite 强行抽象重量级统管客户端，会破坏用户自主权并增加进程间 Session 共享隐患。
