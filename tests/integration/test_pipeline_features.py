...
class TestOrphanLockDetection:
    ...
    def test_lock_file_survives_cleanup_ipc_files(self, tmp_path, monkeypatch):
        ...
        # 模拟 job 完成后的 IPC 清理
        journal.signals_path("t::a").touch()
        journal.result_path("t::a", "abc").touch()
        journal.cleanup_ipc_files("t::a", incarnation="abc")
        # 结果/信号被清理，锁文件保留
        assert not journal.signals_path("t::a").exists()
    ...
    def test_orphan_hold_defers_dispatch(self, tmp_path, monkeypatch):
        ...
        assert "t::a" in [Job.from_dict(j).uid for j in p._runtime.state.queue]
        # (c) 释放锁 → 探测恢复
...
class TestLifecycleHooks:
    ...
    def test_on_run_end_interrupted_reason(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(p._runtime._dispatch, "dispatch_job", exploding)
